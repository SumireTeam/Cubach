...
namespace Cubach.Client
{
    public class LocalServer : IDisposable
    {
        ...
        public LocalServer(Configuration config)
        {
            this.config = config;
        ...
        public void Run(Queue<ServerMessage> serverQueue, Queue<ClientMessage> clientQueue)
        {
            clientManager = new LocalClientManager();
            (clientManager as LocalClientManager).AddConnection(serverQueue, clientQueue);
